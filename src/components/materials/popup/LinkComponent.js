...
function LinkComponent({link, isChecked, handler}) {
  
    return (
        <div key={link.id} className="d-flex justify-content-between align-items-center col-6 mb-3">

            <label className="d-flex w-100 justify-content-between material__card align-items-center pl-2 pt-2 pb-2">
                <div className="d-flex align-items-center">
                    <a target="_blank" rel="noreferrer" href={`http://${link.href}`} className="material__label ml-2">{link.name}</a>
                </div>
